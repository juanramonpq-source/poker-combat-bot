...
const towerControlPositionKey = "pocobot-story-tower-4b-control-position-v1";
...
const towerControlPositionState = {
...
function readTowerControlPosition() {
  try {
    const parsed = JSON.parse(localStorage.getItem(towerControlPositionKey) || "{}");
...
function rememberTowerControlPosition(force = false) {
  const x = Math.round(player.x);
  const y = Math.round(player.y);
  const now = Date.now();
  if (!force) {
    const movedEnough = Math.abs(x - towerControlPositionState.x) >= 8 || Math.abs(y - towerControlPositionState.y) >= 8;
    const enoughTimePassed = now - towerControlPositionState.savedAt >= 900;
    if (!movedEnough && !enoughTimePassed) return;
  }
  towerControlPositionState.x = x;
  towerControlPositionState.y = y;
  towerControlPositionState.savedAt = now;
  try {
    localStorage.setItem(towerControlPositionKey, JSON.stringify({ x, y, savedAt: now }));
  } catch (error) {}
}
...
function applyInitialStorySpawn() {
  ...
  const savedPosition = readTowerControlPosition();
  if (savedPosition) {
    placePlayerAt(savedPosition);
    rememberTowerControlPosition(true);
    return;
  }
...
const collisionZones = [
  ...
  { type: "rect", x: 1016.2295081967211, y: 123.34893893716378, width: 124, height: 208 },
  { type: "rect", x: 1271.7276166456495, y: 375.0873086244606, width: 180, height: 166 },
  { type: "rect", x: 1128, y: 648, width: 166, height: 180 },
  { type: "rect", x: 717.1197982345524, y: 745.7778565939587, width: 334, height: 164 },
  { type: "rect", x: 132, y: 740, width: 172, height: 156 },
  { type: "circle", x: 1133.906683480454, y: 663.6781935331323, radius: 92 },
  {
    type: "poly",
    points: [
      { x: 625.5182849936949, y: 82.46485783531361 },
      { x: 1057.5182849936941, y: 82.46485783531361 },
      { x: 1111.5182849936941, y: 242.46485783531344 },
      { x: 963.5182849936949, y: 322.4648578353135 },
      { x: 697.5182849936949, y: 316.4648578353135 },
      { x: 577.5182849936949, y: 218.46485783531355 },
    ],
  },
  { type: "ellipse", x: 331, y: 208, width: 92, height: 194 },
  { type: "ellipse", x: 377.33291298865066, y: 378.68806525979784, width: 62, height: 116 },
  { type: "ellipse", x: 111, y: 376, width: 108, height: 282 },
  { type: "ellipse", x: 190.64564943253467, y: 584.0235857421528, width: 74, height: 95 },
  { type: "ellipse", x: 99, y: 120, width: 199, height: 234 },
  { type: "ellipse", x: 1145, y: 88, width: 388, height: 161 },
  { type: "ellipse", x: 999, y: 83, width: 303, height: 75 },
  { type: "ellipse", x: 1024, y: 92, width: 191, height: 97 },
];
window.PoCoBOTStoryCollisionEditor?.applySceneZones("tower-control", collisionZones);
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) rememberTowerControlPosition(true);
});
