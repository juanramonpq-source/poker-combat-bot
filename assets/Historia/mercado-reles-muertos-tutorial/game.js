...
async function loadAssets() {
  const [mapImage, veraImage, sparringBot, ...botFrames] = await Promise.all([
...
function update(dt) {
  ...
  player.flameSkewX = damp(player.flameSkewX, -sideSlip * 34, 10, dt);
  player.flameSkewY = damp(player.flameSkewY, -forwardSlip * 18, 10, dt);
...
function drawLoading() {
  ...
  ctx.fillText("Cargando Mercado de Reles Muertos...", 252, 265);

  ctx.fillStyle = "rgba(238, 248, 255, 0.72)";
  ctx.font = "16px Trebuchet MS";
  ctx.fillText("Preparando puestos, cartas y puntos de interaccion", 278, 295);
}
