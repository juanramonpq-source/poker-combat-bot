...
function showNextRadio() {
  ...
  radioState.timer = clamp(9 + next.length * 0.07, 10.4, 21);
  playRadioOpenSound();
}
...
async function loadAssets() {
  const [mapImage, xavorPortraitImage, windowViewImage, unmannedMechaImage, ...botFrames] = await Promise.all([
...
function drawLoading() {
  ...
  ctx.fillText("Cargando sala de control 4B...", 286, 265);
